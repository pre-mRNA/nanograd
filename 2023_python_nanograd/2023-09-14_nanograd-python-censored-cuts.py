...
# saturation function
def saturation(y, window=1):

    # check for bad transcript batches 
    if len(y) == 0 or np.isnan(y).all():
        return np.nan

    # store read length freqs as np array     
    freqs = np.array([np.sum(y == x) for x in range(1, int(np.max(y)) + 1)])
    
    # calculate the difference in cumulative read length between length invervals
    diffs = np.diff(np.cumsum(freqs), prepend=0)

    # find the saturation point, after a window of masked positions 
    return np.argmax(diffs[window:]) + window

# batch function for parallelising saturation calculation 
def process_batch(sub_data, column_to_use, window):
...
def calculate_cuts_per_transcript(merged_data_file, n_threads, censor=False, window=1):
    ...
    # split the data by unique ref_transcript for batching
    unique_transcripts = merged_data['ref_transcript'].unique()
    batches = np.array_split(unique_transcripts, n_threads)

    with ProcessPoolExecutor(max_workers=n_threads) as executor:
        futures = list(executor.map(process_batch, [merged_data[merged_data['ref_transcript'].isin(batch)] for batch in batches], [column_to_use]*n_threads, [window]*n_threads))

    # collect results and concatenate
    final_df = pd.concat(futures).reset_index(name="Saturation_3p").sort_values(by="ref_transcript")

    # report
    elapsed_time = round(time.time() - start, 2)
    logging.info(f'Calculated cuts per transcript in {elapsed_time} seconds')
...
# separate masking function for testing/debugging
...
def main(args):
    ...
    # run the cuts per transcript function if mode = cuts
    elif args.mode == 'cuts':
        logging.info('Calculating cuts per transcript')
        calculate_cuts_per_transcript(merge_temp_file, args.threads) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--threads', type=int, default=1, help='Number of threads to use') ## only implemented for saturation calcs
